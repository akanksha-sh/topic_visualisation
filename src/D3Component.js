...
class D3Component extends React.Component {
  ...
  drawCluster = () => {
    const {topics} = this.state;
      ...
      let dataset = d3.nest().key(function(d) { 
              return d["Category"];
      ...
      groups.forEach(function(d,i) {
        ...
        newObj.name = d;
          let d1 = topics.filter(function(a) { return a.Year == d;})
            newObj.children = [];

            subCat.forEach(function(j,k){
              newObj3 = {};
              var d2 = d1.filter(function(a) { return a['Category'] == j;})
              if(d2.length !== 0){
              newObj3.name = j;
                  d2 = d1.filter(function(a) { return a['Category'] == j;})
                  newObj3.children = [];                      
                  var arr = [];
                  d2.forEach(function(m,n) {
                      if (!arr.includes(m['ClusterId'])) {arr.push(m['ClusterId'])}
        })
        
        arr.forEach(function(p,q){
          var obj4 = {}
          var d3 = d2.filter(e=> {return e.ClusterId == p})
      ...
      if(selectedCategory!='all')
      {
          ...
          if(root2.children[i].children.length==0)
          root2.children.shift()
      }
      }
    if(root2.children.length === 0)
      ...
      .enter().append("g")
        .attr("class", function(d) { return d.children ? "node" : classes.leaf_circle; })
        ...
        .text(function(d) { 
            ...
            if (d.height == 1)
            {
                ...
                {
                    return (d.Year == year) && (d.Category == category) && (d.ClusterId == clusterID)  && (d.TopicId == topId) 
                })
                if(details[0] != undefined)
                return "Keywords: "+details[0].Keywords + "\n" +"Sentiment: " + details[0].Sentiment;              
            }
            else if(d.height == 0)
            {
                if(d.depth == 5){
        ...
        if (d.height == 0) console.log("article node clicked");
      else if (d.height == 1) console.log("topic node clicked");
      else if (d.height == 2) console.log("Cluster", d.data.name, d.parent.data.name, d.parent.parent.data.name);
    })
  }   
    ...
    <>
       <Box p={3} pt={2}>
        ...
        <Box p={3} textAlign='center'>
            <div ref={this.myRef} width="1400" style={{position: 'relative'}}></div>
        </Box>
     </>
    );
  }
